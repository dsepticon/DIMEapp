...
| Record                      | Key/content                                                                                                         | Logical expiry and TTL                                                           |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
...
| OAuth login                 | Existing `AUTH#v1#login:<state digest>`; encrypted browser digest, OAuth nonce, expiry, temporary tester tag/expiry | Earlier of 5 minutes or invitation expiry; deleted on valid callback/denial      |
| Tester session              | Existing `AUTH#v1#session:<session digest>`; existing fields plus `testerUntil`                                     | 8 hours absolute, 30 minutes idle; epoch rotation revokes old sessions           |
| Provider grant              | Existing encrypted `grant:<OAuth subject>`                                                                          | At most session absolute expiry (8 hours); existing revocation rules             |
| Link intent                 | Existing encrypted exact-key record; one active intent per session                                                  | 5 minutes                                                                        |
| Link outcome                | Existing encrypted record plus tester deadline                                                                      | 24 hours; TESTERS replay also checks deadline                                    |
| Gameplay receipt            | Existing player partition/class, unchanged                                                                          | 30 days                                                                          |
| Deletion tombstone/recovery | Existing bounded deletion state machine                                                                             | 35 days                                                                          |
...
- The owner bridge compensates for the installed resolver’s argument-only implementation: it loads only pinned, hash-verified resolver bytes and passes the result solely in the signing child environment. It strips AWS/debug environment variables from that child and suppresses upstream/child failure diagnostics; no raw key enters command arguments, files or reports. Synthetic bridge tests cover these boundaries.
