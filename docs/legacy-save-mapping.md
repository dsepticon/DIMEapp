...
The preview is deterministic for the same source, has no I/O, clones its input, and produces a source SHA-256 for retry comparison. This digest identifies the serialized input, not an authenticated player or a tamper-proof record. A three-item conditional DynamoDB migration transaction is prepared but not invoked by the Lambda: it writes v2 state, a player migration receipt and a global source receipt without TTL, while leaving the source record unchanged. It is not enabled because no verified legacy reader, identity link or complete candidate mapper exists. The retry tests prove the offline preview and mocked transaction shape, not live conversion. `server/migration-gate.ts` is a read-only assessment gate: existing v2 state wins, duplicate channel sources produce reconciliation, and uncertain links/mappings preserve source without creating a save. It is not wired to the Lambda and writes nothing.
...
Approved policy: one global save per verified persistent Twitch player, keyed `PLAYER#v1#<server HMAC of signed U opaque_user_id>`, independent of channel. Legacy localStorage is browser/origin scoped, and legacy DynamoDB keys do not prove binding to the signed global identity. Channel ownership alone never proves a player link. Moving from the custom domain to a Twitch asset origin prevents direct access to old localStorage: a deliberate export/import or authenticated legacy-origin bridge is required. Do not silently create new profiles across that boundary.
...
Release remains blocked on the decoder/legacy-origin bridge, identity policy, anonymized DynamoDB schema, unresolved order/claim mapping, complete compatibility reader and transactional retry/concurrency tests. The current new-player load path does not discover legacy or older channel-scoped v2 records; do not cut over existing players to it yet. A verified identity link, complete source discovery and conflict check must precede a global save initialization. Existing v2 saves are read as v2 and never remigrated. Multiple candidate channel records enter reconciliation without adding balances or inventories; originals remain unchanged. No production migration write path is enabled. The prepared transaction is unreachable from the Lambda; no bulk migration or source mutation exists.
