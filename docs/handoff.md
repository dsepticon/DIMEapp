September 12 staging-source validation: Node v22.23.2 `npm ci` and `npm run check` passed 68/68 Vitest tests plus lint/typecheck/frontend and Lambda bundle; Playwright passed 5/5; format and diff checks passed. CloudFormation ValidateTemplate accepted the corrected template and all eight parameters after AWS CLI authentication was renewed. Live Amplify automatic creation patterns remained the literal `main` and `codex/react-extension-rebuild`, with no review branch connected. SAM CLI, cfn-lint and cfn-guard were unavailable; no processed template or change set was created. The Lambda bundle excludes migration code. No AWS/Twitch resource was modified. A follow-up review removed unsupported `aws:SourceArn`/`aws:SourceAccount` conditions from the Lambda execution-role trust; only the Lambda service principal may assume it. Dynamic references resolve into encrypted Lambda environment configuration at deployment, so `lambda:GetFunctionConfiguration` must remain restricted. See the [staging guide](../infra/staging/README.md) for the trust and resource-policy boundary.
