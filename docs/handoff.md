September 13 Hosted Test diagnosis: the owner reports the corrected v2 ZIP is uploaded to Twitch `0.4.0`; its panel loads, but four authenticated `GET /state` requests returned Lambda `NOT_FOUND`/HTTP 404. The backend compared a named-stage event path against `/state` without stripping the exact `staging` segment. The raw path was not logged, so the precise event value is inferred and reproduced locally with synthetic data. A narrow handler correction and tests now normalize only `/{requestContext.stage}` at a complete segment boundary. No frontend change or replacement ZIP is needed. The source fix is not yet deployed; review the proposed CloudFormation UPDATE before any execution. See [staging verification](staging-verification.md).
