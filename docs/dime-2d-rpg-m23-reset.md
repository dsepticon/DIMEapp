...
The historical Milestone 2.3 renderer limitation above is addressed in the uncommitted Milestone 3 branch: the server factory and reset result now include `world.zone=ARC_L1_START`, an arrival entry, no node exceptions, no scanner/mining session, no active ROC, and empty fractional cargo remainders. The visible renderer selects ARC-L1 from that returned server snapshot. Old pending actions remain fenced by the reset generation and monotonic revision. This is a draft integration, not a released reset contract; see [Milestone 3](dime-2d-rpg-m3-world-mining.md) for its remaining validation and world-design gaps.
