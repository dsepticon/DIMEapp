...
The deployer/CloudFormation execution role needs permission to resolve both existing secrets, plus `kms:Decrypt` if they use customer-managed KMS keys. The Lambda execution role does **not** need Secrets Manager access: versionless CloudFormation dynamic references populate its encrypted environment at deployment. A secret value change alone does not refresh Lambda configuration; a reviewed function update is needed. Rotate the Twitch signing secret without changing `PlayerIdentityKeySecretArn` or its value. Never put secret values in parameter files, outputs, logs, commands, Git, or chat. The Lambda environment contains resolved secrets, so limit `lambda:GetFunctionConfiguration` and Lambda-console access to approved operators. Keep the signing and identity-key secrets separate.
