...
resources['EbsExecutionRole']['Properties']['Policies'][0]['PolicyDocument']['Statement'].append({'Sid':'CheckCanonicalBindingAndSave','Effect':'Allow','Action':['dynamodb:ConditionCheckItem'],'Resource':table,'Condition':{'ForAllValues:StringLike':{'dynamodb:LeadingKeys':['BINDING#v1#*','PLAYER#v1#*','ACCOUNT#v1#*','CONTROL#v1#*']},'Null':{'dynamodb:LeadingKeys':'false'}}})
...
resources['WebAuthExecutionRole']={'Type':'AWS::IAM::Role','Properties':{'AssumeRolePolicyDocument':copy.deepcopy(resources['EbsExecutionRole']['Properties']['AssumeRolePolicyDocument']),'Policies':[{'PolicyName':'ScopedWebAuthenticationAndCanonicalState','PolicyDocument':{'Version':'2012-10-17','Statement':[
 {'Sid':'WebAuthAndCanonicalTransactions','Effect':'Allow','Action':['dynamodb:GetItem','dynamodb:PutItem','dynamodb:ConditionCheckItem'],'Resource':table,'Condition':{'ForAllValues:StringLike':{'dynamodb:LeadingKeys':['AUTH#v1#*','BINDING#v1#*','PLAYER#v1#*','ACCOUNT#v1#*','CONTROL#v1#*']},'Null':{'dynamodb:LeadingKeys':'false'}}},
 {'Sid':'DeleteManifestOwnedRecords','Effect':'Allow','Action':['dynamodb:DeleteItem'],'Resource':table,'Condition':{'ForAllValues:StringLike':{'dynamodb:LeadingKeys':['AUTH#v1#*','BINDING#v1#*','PLAYER#v1#*','ACCOUNT#v1#*','CONTROL#v1#*']},'Null':{'dynamodb:LeadingKeys':'false'}}},
...
f['Environment']['Variables'].update({'DIME_WEB_ORIGIN':'https://destroyaindustriesminingextension.com','DIME_OAUTH_CLIENT_ID':{'Ref':'OAuthClientId'},'DIME_ACCOUNT_LINKING':{'Ref':'AccountLinkingMode'},'DIME_WEB_SIGN_IN_MODE':{'Ref':'WebSignInMode'}})
# Web-auth-only refresh marker: never bump the shared gameplay configuration parameter.
f['Environment']['Variables']['DIME_AUTH_KEY_REVISION']='web-key-repair-20260918-1'
...
routes=[('GET','/auth/login'),('GET','/auth/callback'),('GET','/auth/session'),('POST','/auth/logout'),('POST','/auth/link/intent'),('POST','/auth/link/accept'),('GET','/api/v4/state'),('POST','/api/v4/actions'),('POST','/api/v4/content/preview'),('POST','/api/v4/content/convert'),('POST','/api/v4/profile/reset'),('POST','/auth/unlink'),('POST','/auth/delete/intent'),('POST','/auth/delete/resume'),('GET','/auth/status')]
