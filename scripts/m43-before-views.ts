/** Capture the preserved compiled 0.8.0 baseline with synthetic state. */
...
const server = spawn(
  ...
  [
    ...
    '4193',
    '--outDir',
    '/tmp/dime-m43-release/before-client',
  ],
  { stdio: 'ignore' },
);
  ...
  for (const layout of [
    ...
    for (const mining of [false, true]) {
      ...
      await page.screenshot({
        path: `/tmp/dime-m43-release/screenshots/before-${layout.name}-${s.world.zone}.png`,
      });
    }
