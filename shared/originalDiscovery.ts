...
export function populateOriginalZone(source: OriginalPlayerState): OriginalPlayerState {
  ...
  if (
    !zone?.regionCount ||
    Object.values(state.world.nodes).some((node) => node.id.startsWith(`${zone.id}.node.`))
  )
    return state;
  const map = originalZoneMap(zone.id);
  const reachable = originalReachableCells(map).queue;
  const cells = reachable.filter((tile) => {
    if (Math.hypot(tile.x - map.spawn.x, tile.y - map.spawn.y) <= 5) return false;
    const candidates = reachable.filter((other) => {
      const d = Math.hypot(other.x - tile.x, other.y - tile.y);
      return d >= 1 && d <= 4;
    });
    const placed: typeof candidates = [];
    for (const candidate of candidates)
      if (placed.every((old) => Math.hypot(old.x - candidate.x, old.y - candidate.y) >= 1.5))
  ...
  for (let index = 0; index < count; index++) {
    ...
    const tile =
      cells.find(
        (candidate, offset) =>
          offset >= start && chosen.every((old) => Math.hypot(old.x - candidate.x, old.y - candidate.y) >= 3),
      ) ??
      cells.find((candidate) =>
        chosen.every((old) => Math.hypot(old.x - candidate.x, old.y - candidate.y) >= 3),
      ) ??
      map.spawn;
    chosen.push(tile);
    ...
    state.world.nodes[id] = {
      ...
      x: tile.x,
      y: tile.y,
      size: 1 + Math.floor(random(seed, index * 7 + 2) * 5),
...
export function refreshOriginalRespawns(source: OriginalPlayerState, now: number): OriginalPlayerState {
  ...
  for (const node of Object.values(state.world.nodes))
    if (node.respawnAt !== null && node.respawnAt <= now) {
      node.status = 'INTACT';
      node.fragments = [];
      node.respawnAt = null;
    }
...
export function analyzeOriginalNode(source: OriginalPlayerState, nodeId: string): OriginalPlayerState {
  ...
  if (!node || node.status !== 'INTACT' || !state.world.scanner.scannedZones?.includes(state.world.zone))
