...
type View = 'game' | 'menu' | 'cargo' | 'refinery' | 'market' | 'profile' | 'travel' | 'mining';
      ...
      {state && (
        <>
          <RpgPanel
            ...
            busy={busy}
            overlay={view !== 'game' || (!!pending && !busy)}
            navigate={setView}
          />
          {(view !== 'game' || (!!pending && !busy)) && (
            <div className={styles.overlayBackdrop}>
              <section className={styles.gameOverlay} role="dialog" aria-label={`${view} menu`}>
                ...
                {view === 'menu' && (
                  <nav className={styles.pauseMenu} aria-label="Game menu">
                    {(['cargo', 'profile', 'refinery', 'market', 'travel', 'mining'] as View[]).map(
                      (entry) => (
                        <button key={entry} onClick={() => setView(entry)}>
                          {entry === 'cargo'
                            ? 'Backpack · Cargo'
                            : entry === 'profile'
                              ? 'Character · Profile'
                ...
                {storageError && (
                  <p className={styles.hint}>
                    Transactions are disabled until local retry storage is repaired.
                  </p>
                )}
                {view === 'mining' && (
                  <MiningPanel
                ...
                {view === 'cargo' && (
                  <section className={styles.content}>
                    ...
                    <p>
                      Mining hold: {scu(total(state.mining[source]))} / {scu(CAPACITIES[source])} SCU. Cargo
                      ship: {scu(total(hold?.raw ?? {}) + total(hold?.refined ?? {}))} /{' '}
                      {scu(CAPACITIES[ship])} SCU.
                    </p>
                ...
                {view === 'refinery' && (
                  <section className={styles.content}>
                    ...
                    <p className={styles.hint}>
                      At ARC-L1, refine raw ore in a Prospector or Mole hold. Collect finished orders into a
                      cargo ship, then sell at Area-18.
                    </p>
                    <div className={styles.refineForm}>
                      {sourceSelect}
                      {oreSelect}
                ...
                {view === 'market' && (
                  <section className={styles.content}>
                    ...
                    <p className={styles.hint}>
                      Sell raw minerals at ARC-L1. Sell gems and refined material at Area-18. The supply shop
                      is at Area-18; purchased ships await you at ARC-L1.
                    </p>
                    <div className={styles.refineForm}>
                      {shipSelect}
                      {oreSelect}
                    ...
                    <p>
                      Indicative value: {format(Math.round((count * ORES[ore][category]) / 100))} aUEC. Your
                      wallet updates when the sale completes.
                    </p>
                    <h2>Supply shop</h2>
                    <div className={styles.refineForm}>
                      <label>
                ...
                {view === 'profile' && (
                  <section className={styles.content}>
                    <h1>Profile & fleet</h1>
                    <p>
                      {config.local ? 'Local test profile' : 'Twitch identity verified by the backend'} ·
