...
export async function webSession() {
  ...
  const value = (await response.json()) as { identity?: unknown; csrf?: unknown; linkingAvailable?: unknown };
  if (typeof value.identity !== 'string' || typeof value.csrf !== 'string')
    throw Error('Sign in with Twitch to continue.');
  return { identity: value.identity, csrf: value.csrf, linkingAvailable: value.linkingAvailable === true };
}
