...
import { originalZoneMap, originalReachableCells } from '../../shared/originalWorld';
...
import { scannerCandidates } from '../../shared/originalScanner';
import type { Facing } from '../../shared/originalNodeTargeting';
export async function journey(page: Page, touch: boolean, width: number, cycleOnly = false) {
  ...
  if (cycleOnly) {
    await go('zone.z017');
    const cycleMap = originalZoneMap('zone.z017');
    let cycled = false;
    outer: for (const point of originalReachableCells(cycleMap).queue) {
      for (const dx of [0.7, 0.9, 0.5, 0.3, 0.1])
        for (const dy of [0.7, 0.9, 0.5, 0.3, 0.1]) {
          for (const facing of ['N', 'E', 'S', 'W'] as Facing[]) {
            const desired = { x: point.x + dx, y: point.y + dy };
            if (scannerCandidates(model.snapshot(), desired, facing).length < 2) continue;
            await walkTo(page, cycleMap, point, touch);
            await canvas.focus();
            // Real input only: reach a sub-tile overlap between the two scanner cones.
            for (const axis of ['x', 'y'] as const) {
              for (let attempt = 0; attempt < 40; attempt++) {
                const actual = Number(
                  await canvas.getAttribute(axis === 'x' ? 'data-player-x' : 'data-player-y'),
                );
                const delta = desired[axis] - actual;
                if (Math.abs(delta) < 0.04) break;
                await page.keyboard.press(axis === 'x' ? (delta > 0 ? 'd' : 'a') : delta > 0 ? 's' : 'w', {
                  delay: Math.max(12, Math.min(45, Math.abs(delta) * 150)),
                });
              }
            }
            await page.keyboard.press({ N: 'w', E: 'd', S: 's', W: 'a' }[facing], { delay: 20 });
            await page.getByRole('button', { name: 'Ping (P)', exact: true }).click();
            await page.waitForTimeout(100);
            const cycle = page.getByRole('button', { name: 'Next scanner target (Q)', exact: true });
            if (!(await cycle.isVisible())) continue;
            const previous = await page.locator('.scannerHUD').getAttribute('data-focus');
            if (touch) await cycle.tap();
            else await page.keyboard.press('q');
            await expect(page.locator('.scannerHUD')).not.toHaveAttribute('data-focus', previous!);
            cycled = true;
            break outer;
          }
        }
  ...
  await page.screenshot({ path: `/tmp/dime-m43-game-publication/screenshots/analyzed-${width}.png` });
  ...
  await page.screenshot({ path: `/tmp/dime-m43-game-publication/screenshots/fracture-${width}.png` });
